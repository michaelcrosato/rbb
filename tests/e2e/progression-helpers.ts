...
export async function grantMaterials(page: Page, inventory: Inventory): Promise<void> {
  ...
  for (const [item, quantity] of Object.entries(inventory)) {
    ...
    while (remaining > 0) {
      ...
      await page.getByRole('button', { name: 'Grant item', exact: true }).press('Enter');
      await expect
...
export async function placePiece(
  ...
  // Palette selection resumes pointer capture. Keep its normal mouse path so a
  // fast keyboard activation cannot race the following drag-to-look gesture.
...
export async function craftRecipe(page: Page, id: RecipeId, count = 1): Promise<void> {
  ...
  await page.locator(`[data-action="craft"][data-value="${id}"]`).press('Enter');
  await expect
