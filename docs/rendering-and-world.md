...
Save version 4 persists environment, tuning, oxygen, species, developer flags, terrain edits and progression state (sites, containers, equipment and graded structures). Explicit v1/v2/v3 migrations retain the existing island and progress; v1 migration also seeds the added species. World-generation version remains 1. Network protocol is now 4: update clients and dedicated servers together. Old clients are rejected cleanly rather than interpreting the new state incorrectly. The [terrain guide](terrain.md) describes volumetric caves, sculpting, dirty mesh updates and terrain replication.
