...
The progression expansion uses network protocol 4. Upgrade the static client and dedicated server together. Save v1, v2 and v3 migrate to v4 while preserving the existing island and progress; take the normal stopped-server backup before upgrading a persistent deployment. Terrain edits, landmark depletion, storage, equipment, structure grades and supports persist in the same SQLite snapshots. New sites conflicting with existing bases or edits are suppressed during migration.
