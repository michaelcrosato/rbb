...
export function habitatValid(
  ...
  const h = terrainHeight(x, z, world.hash);
  if (WILDLIFE[species].habitat === 'sea') return h < (species === 'dolphin' ? -4 : -2);
  return (
    h > 1.5 &&
    Math.abs(terrainHeight(x + 1, z, world.hash) - h) < 1.2 &&
    Math.abs(terrainHeight(x, z + 1, world.hash) - h) < 1.2
  );
}
...
export function spawnWildlife(
  ...
  for (
    ...
    if (
      !habitatValid(world, species, ax, az) ||
      state.buildings.some((b) => distance2(b, { x: ax, z: az }) < 4)
    )
      continue;
    state.animals.push(animalAt(world, species, `a${state.nextId++}`, ax, az));
    spawned++;
  }
  return spawned;
}
  ...
  for (const animal of state.animals) {
    ...
    if (dist > (chase ? 1.5 : 0.4)) {
      ...
      for (const offset of [0, Math.PI / 2, -Math.PI / 2]) {
        ...
        const terrain = terrainHeight(x, z, world.hash);
        if (
          !habitatValid(world, animal.species, x, z) ||
          (!sea &&
            (groundHeight(state, world, x, z) - terrain > 0.2 ||
              state.buildings.some((b) => distance2(b, { x, z }) < (b.kind === 'wall' ? 2.3 : 1))))
        )
          continue;
        if (
          !sea &&
    ...
    const ground = terrainHeight(animal.x, animal.z, world.hash);
    animal.y = sea
      ? clamp(
          -1.1 +
    ...
    if (
      ...
      animal.cooldown <= 0 &&
      lineOfSight(state, target, animal.x, animal.y + 0.5, animal.z)
    ) {
